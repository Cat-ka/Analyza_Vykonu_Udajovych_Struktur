...
namespace structures
{
	...
	private:
		...
		size_t size_;  //rovnakÈ ako unsigned long long - pre windowsy long long sa nahr·dza slovom size_t
	...
	template<typename T>
	Array<T>::Array(const size_t size) :    //NIKDY NEVYTV¡RAç äTRUKT⁄RU S TYPOM OBJEKT! 
	...
	Array<T>::~Array()
	{
		delete this->vector_; //keÔ alokujem cez new, maûem cez delete, keÔ alokujem cez malloc, maûem cez free
	...
	T& Array<T>::operator[](const int index)
	{
		...
		int indexVector = this->mapFunction(index);
		return *(reinterpret_cast<T*>(vector_->getBytePointer(indexVector)));
	}
	...
	const T Array<T>::operator[](const int index) const
	{
		...
		T pom = *(reinterpret_cast<T*>(this->vector_->getBytePointer(vectorIndex)));
		return pom;  // len ako uk·ûka, lepöie je norm·lne jak vyööie v metÛde
	}
	...
	inline bool Array<T>::operator==(const Array<T>& other) const
	{
		...
		return *this->vector_ == *other.vector_; //&& this->size_ == other.size_; nemusÌme porovnaù veækosti, uû s˙ v triede vektor porovn·vanÈ
	}

	template<typename T>
	void Array<T>::copy(const Array<T>& src, const int srcStartIndex, Array<T>& dest, const int destStartIndex, const int length)
	{
		int srcStartIndexVector = src.mapFunction(srcStartIndex);
		int destStartIndexVector = dest.mapFunction(destStartIndex);
	...
	{
		return sizeof(T) * index;  //(i-K) * sizeof(T) + A vöeobecn˝ vzorec
	}
}
